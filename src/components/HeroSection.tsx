...
export default function HeroSection() {
  ...
  return (
    // Main Content
    <div className="w-full h-full pt-20 text-center flex flex-col lg:flex-row">

      {/* Left Side - Text Content */}
      <div className="md:text-start w-full lg:w-1/2 lg:pr-12">
        <div className={`transform transition-all duration-1000 ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'
          ...
          {/* Feature Pills */}
          <div className="hidden w-fit text-center md:flex flex-wrap justify-center lg:justify-start gap-4 mb-10">
            {features.map((feature, index) => (
              <div
                key={index}
                className={`transform transition-all flex duration-500 hover:scale-105 ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'
                  }`}
                style={{ transitionDelay: `${index * 100}ms` }}
              >
                <div className="text-center p-2 mx-auto bg-white/10 backdrop-blur-md rounded-full px-4 py-2 border border-white/20 hover:bg-white/20 transition-all">
      ...
      <div className="w-full flex flex-col mx-auto lg:w-1/2 mt-16 lg:mt-0">
        <div className={`w-full transform transition-all duration-1200 ${isLoaded ? 'translate-x-0 opacity-100' : 'translate-x-8 opacity-0'
          }`}>
